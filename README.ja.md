...
# 🦁 XXD Panel 083｜ブサかわキャラ手帳

### 写真の性格を少なく、変で、不器用で、正確な丑萌えキャラクターへ
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 083 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真全体を描き直すと退屈な線画になり、きれいで正しく描きすぎると対象の面白い性格、動作、対比、気まずさが消えます。

**Panel 083** は記憶に残る主体一つと必要な環境暗示を最大二つだけ残し、比率、動作、表情、構造のどれかを制御して歪めます。震える輪郭、意図的な比率誤差、粗いクレヨン、余白が Ugly-cute naïve doodle を作り、同一性とユーモアは必ず元写真から導きます。

### 向いている用途

- 性格のある動作、表情、比率、対比をユーモアへ変えたい写真
- わざと不器用で歪みながら、一般的な可愛さや識別不能にはしたくない場合
- 複雑な場面を一主体、少数の暗示、粗い色、大きな余白へ減らしたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 083 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** Ugly-cute 素朴な落書き · Wonky 手描き · 制御された不正確さ · 一人のユーモア主役 · 粗いクレヨン · 少・変・不器用・正確
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 独立出版の丑可愛い落書き小冊子のようなユーモラスなキャラクターポスター |
| **識別しやすい特徴** | 震える不均一輪郭、意図的な比率誤差、少量の粗い色、一主体、大きな淡色余白 |
| **入力の尊重** | 同一性と笑いは元写真から取り、最も性格的な動作、対比、表情、構造だけを誇張する |
...
```text
描く前に写真を再解釈 → 記憶に残る主体一つと必要な暗示を最大二つだけ残す → 最も面白い動作・対比・表情・比喩を選ぶ → 制御された誤りと wonky 輪郭で誇張 → 元色を粗いクレヨンへ簡略化 → 淡い余白を残す → 必要な場合だけ一言の注記を加える
```

## 完成作の特徴

- 主役は一つだけで、環境物は本当に必要な場合以外すべて削除する。
- 比率、遠近、輪郭は意図的に誤るが、誤りは制御され識別可能である。
- クレヨン、色鉛筆、ドライブラシ、塗り残し、はみ出し、紙粒子で私的な小冊子の表面を作る。
- 拡大、縮小、偏心、裁ち落とし、浮遊で主体の喜劇的性格を強める。
- 文字は情報システムではなく、小さなツッコミ、観察、傍白として振る舞う。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-083-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 083 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-083 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-083 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-083 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-083 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 083 を適用し、共通設定は一度だけ解決
/xxd-panel-083 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-083`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-083-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-083-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

83 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 083 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–083） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
