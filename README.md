<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 083 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 083 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
重新理解照片 → 提取一个视觉核心与准确身份 → 找到最有趣的性格、动作、反差或隐喻 → 控制性夸张比例、表情或结构 → 用发抖、歪斜、不均匀轮廓故意画笨 → 只保留至多两个环境暗示 → 从原图挑少数精神色用蜡笔、彩铅、干刷粗糙上色 → 以留白和一句可选吐槽完成叙事
```

## 成品中最容易识别的风格特点

- 画得太正确就是失败；不准确必须经过控制，并服务于准确识别与幽默。
- 页面只保留一个主体和至多一两个必要环境暗示，不做生活物品清单。
- 构图不复制照片，可放大、缩小、偏心、悬置、裁切，并用留白强化主体性格。
- 蜡笔、彩铅、干刷或粗糙平涂允许越线、漏白和摩擦颗粒，避免商业精修。
- 文字仅在增强性格时出现，像作者随手补上的吐槽、观察或旁白。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-083-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–083） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
