...
# 🦁 XXD Panel 083｜못생기고 귀여운 캐릭터 저널

### 사진의 성격을 적고, 이상하고, 서툴지만 정확한 어글리 큐트 캐릭터로
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 083이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진 전체를 다시 그리면 지루한 선화가 되고 너무 예쁘고 정확하게 그리면 대상의 흥미로운 성격, 동작, 대비, 어색함이 사라집니다.

**Panel 083**은 기억에 남는 피사체 하나와 필요한 환경 암시 최대 두 개만 남기고 비율, 동작, 표정, 구조 중 하나를 통제해서 비틉니다. 떨리는 윤곽, 의도적 비율 오차, 거친 크레용, 여백이 Ugly-cute naïve doodle을 만들며 정체성과 유머는 반드시 원본에서 나옵니다.

### 이런 경우에 적합합니다

- 성격 있는 동작, 표정, 비율, 대비를 유머로 바꾸고 싶은 사진
- 일부러 서툴고 비뚤지만 일반적인 귀여움이나 식별 불능은 피하고 싶을 때
- 복잡한 장면을 한 피사체, 소수 단서, 거친 색, 넓은 여백으로 줄이고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 083로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** Ugly-cute 순진한 낙서 · Wonky 손그림 · 통제된 어긋남 · 하나의 코믹 주인공 · 거친 크레용 · 적고 이상하고 서툴고 정확하게
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 독립 출판 못난이 귀여운 낙서 소책자 같은 유머러스한 캐릭터 포스터 |
| **대표 특징** | 떨리는 불균일 윤곽, 의도적 비율 오차, 소량의 거친 색, 한 피사체, 넓은 옅은 여백 |
| **입력 존중 방식** | 정체성과 웃음은 원본에서 가져오고 가장 성격적인 동작, 대비, 표정, 구조만 과장함 |
...
```text
그리기 전 사진 재해석 → 기억에 남는 피사체 하나와 필요한 단서 최대 두 개만 유지 → 가장 웃긴 동작·대비·표정·은유 선택 → 통제된 오류와 wonky 윤곽으로 과장 → 원본 색을 거친 크레용으로 단순화 → 옅은 여백 유지 → 필요할 때만 한마디 메모 추가
```

## 완성작의 특징

- 주인공은 하나이며 환경물은 정말 필요한 경우 외에는 모두 삭제합니다.
- 비율, 원근, 윤곽은 의도적으로 틀리지만 오류는 통제되고 알아볼 수 있습니다.
- 크레용, 색연필, 드라이브러시, 덜 채운 색, 선 넘김, 종이 입자로 사적 소책자 표면을 만듭니다.
- 확대, 축소, 편심, 크롭, 부유로 피사체의 희극적 성격을 강화합니다.
- 글자는 정보 체계가 아니라 작은 투정, 관찰, 방백처럼 작동합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-083-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 083의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-083 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-083 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-083 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-083 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-083 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-083 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 083 적용, 공통 설정은 한 번만 확정
/xxd-panel-083 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-083`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-083-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-083-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

83개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 083까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–083) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
